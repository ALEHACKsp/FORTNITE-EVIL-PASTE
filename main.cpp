...
__declspec(dllexport) HRESULT PresentHook(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags) {
	...
	if (!device) {
		...
		ImGui::GetIO().Fonts->AddFontFromFileTTF(("C:\\Windows\\Fonts\\arial.ttf"), 12.0f);
	...
	if (ShowMenu)
	{
		...
		static int tabb = 0;
		ImGui::Begin((" Fortnite Evil "), reinterpret_cast<bool*>(true)); // start open
		ImGui::SetWindowPos(ImVec2(0, 25), ImGuiCond_FirstUseEver);
		    
		ImGui::Text("coded-by: * btwttv *");
		ImGui::Text("aimbot-bind: right-mouse");

			ImGui::Text("[ban risk]");
			if (Settings.Aimbot) {
				ImGui::Checkbox(("shot-through-walls (snipers-only)"), &Settings.AutoAimbot);
				ImGui::Checkbox(("no-spread(every-weapon)"), &Settings.NoSpreadAimbot);
				ImGui::Checkbox(("no-reload(every-weapon)"), &Settings.InstantReload);
				ImGui::Checkbox(("memory-aimbot"), &Settings.Aimbot);
				ImGui::Checkbox(("silent-aimbot"), &Settings.SilentAimbot);
				if (!Settings.AutoAimbot) {
					ImGui::Checkbox(("fov##checkbox"), &Settings.ESP.AimbotFOV);
				}
				if (!Settings.AutoAimbot) {
					ImGui::SliderFloat(("fov##slider"), &Settings.AimbotFOV, 0.0f, 1000.0f, ("%.2f"));
					ImGui::SliderFloat(("aim-smoothing##slider"), &Settings.AimbotSlow, 0.0f, 25.0f, ("%.2f"));
			...
			if (Settings.ESP.Players) {
				ImGui::Checkbox(("snap-lines"), &Settings.ESP.PlayerLines);
				ImGui::Checkbox(("name"), &Settings.ESP.PlayerNames);

				ImGui::PushItemWidth(150.0f);
				ImGui::ColorPicker3(("visible"), Settings.ESP.PlayerVisibleColor);
				ImGui::ColorPicker3(("not-visible"), Settings.ESP.PlayerNotVisibleColor);
				ImGui::PopItemWidth();
				ImGui::Spacing();
				ImGui::SliderFloat(("fov-changer"), &Settings.FOV, 60.0f, 160.0f, ("%.2f"));
			}
			...
			if (Settings.ESP.Weapons) {
				ImGui::SliderInt(("min weapon tier"), &Settings.ESP.MinWeaponTier, 0, 8);
			}
		ImGui::End();
	}
